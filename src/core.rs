...
impl Difficulty {
    /// Board size as (width, height) in Cells.
...
impl Game {
    ...
    /// Creates a game with a preset Mine list (test / reproduction use).
    /// The caller is responsible for first-click safety: Mines on or adjacent
    /// to the first-clicked Cell are NOT filtered.
    #[allow(dead_code)] // public API for tests and future AI/replay use
    pub fn with_mines(difficulty: Difficulty, mines: &[(usize, usize)]) -> Self {
    ...
    pub fn is_trigger(&self, row: usize, col: usize) -> bool {
        self.trigger == Some((row, col))
    }

    /// The visible state of a Cell.
    pub fn cell_state(&self, row: usize, col: usize) -> CellView {
        let idx = self.index(row, col);
        let state = self.cells[idx];
        let content = match state {
            CellState::Revealed => Some(if self.is_mine(row, col) {
                CellContent::Mine
            } else {
                CellContent::Number(self.adjacent_mines(row, col))
            }),
            _ => None,
        };
    ...
    /// Total Mines minus Flags placed.
    ...
    /// or the coordinates are out of bounds. The first Reveal of a game
    /// places the Mines (ADR-0001) and starts the clock.
    pub fn reveal(&mut self, row: usize, col: usize) {
        ...
        self.reveal_at(row, col);
        self.resolve_end(row, col);
    }
    ...
    fn reveal_at(&mut self, row: usize, col: usize) {
        ...
        if self.adjacent_mines(row, col) == 0 {
            ...
            while let Some((r, c)) = queue.pop_front() {
                for (nr, nc) in Self::neighbors(self.width, self.height, r, c) {
                    let nidx = self.index(nr, nc);
                    if self.cells[nidx] != CellState::Hidden || self.is_mine(nr, nc) {
                        continue;
                    }
                    self.cells[nidx] = CellState::Revealed;
                    if self.adjacent_mines(nr, nc) == 0 {
            ...
            None => {
                ...
                for row in 0..self.height {
                    for col in 0..self.width {
                        let excluded = (row, col) == (first_row, first_col)
                            || (row.abs_diff(first_row) <= 1 && col.abs_diff(first_col) <= 1);
                        if !excluded {
    ...
    fn resolve_end(&mut self, row: usize, col: usize) {
    ...
    fn lose(&mut self, row: usize, col: usize) {
            ...
            (0..self.height).all(|row| {
                (0..self.width).all(|col| {
                    mines.contains(&(row, col))
                        || self.cells[self.index(row, col)] == CellState::Revealed
                })
            })
        });
    ...
    fn is_mine(&self, row: usize, col: usize) -> bool {
        self.mines
            .as_ref()
            .is_some_and(|mines| mines.contains(&(row, col)))
    }

    fn adjacent_mines(&self, row: usize, col: usize) -> u8 {
        Self::neighbors(self.width, self.height, row, col)
    ...
    fn neighbors(
        ...
        let (min_r, max_r) = (row.saturating_sub(1), (row + 1).min(height - 1));
        let (min_c, max_c) = (col.saturating_sub(1), (col + 1).min(width - 1));
        (min_r..=max_r).flat_map(move |r| (min_c..=max_c).map(move |c| (r, c)))
    }

    /// Toggles a Flag on a Hidden Cell. No-op otherwise: Revealed Cells and
    /// ended games reject Flag toggling.
    ...
    pub fn chord(&mut self, row: usize, col: usize) {
        if self.ended() {
            return;
        }
        if !self.in_bounds(row, col) {
        ...
        if self.cells[self.index(row, col)] != CellState::Revealed {
            return;
        }
        let number = self.adjacent_mines(row, col);
        if number == 0 {
            return;
        }
        let flagged = Self::neighbors(self.width, self.height, row, col)
            .filter(|&(r, c)| self.cells[self.index(r, c)] == CellState::Flagged)
            .count();
        if flagged != number as usize {
            return;
        }
        let mut hit_mine = None;
        for (nr, nc) in Self::neighbors(self.width, self.height, row, col) {
            let nidx = self.index(nr, nc);
            if self.cells[nidx] == CellState::Hidden {
                self.cells[nidx] = CellState::Revealed;
                if self.is_mine(nr, nc) && hit_mine.is_none() {
                    hit_mine = Some((nr, nc));
                }
            }
        }
        match hit_mine {
            Some((r, c)) => self.lose(r, c),
            None => self.check_win(),
        }
    }
...
mod tests {
    ...
    fn new_game_starts_ready_with_all_cells_hidden() {
        ...
        for row in 0..height {
            for col in 0..width {
                assert_eq!(game.cell_state(row, col).state, CellState::Hidden);
                assert_eq!(game.cell_state(row, col).content, None);
            }
        }
    }
    ...
    fn reveal_mine_loses_and_auto_reveals_board() {
        let mut game = Game::with_mines(Difficulty::Beginner, &[(0, 0), (5, 5)]);
        game.reveal(0, 0);
        assert_eq!(game.game_state(), GameState::Lost);
        assert!(game.is_trigger(0, 0));
        // The Trigger Mine is Revealed and shown as Mine.
        assert_eq!(game.cell_state(0, 0).state, CellState::Revealed);
        assert_eq!(game.cell_state(0, 0).content, Some(CellContent::Mine));
        // The other unflagged Mine is auto-Revealed too.
        assert_eq!(game.cell_state(5, 5).state, CellState::Revealed);
        assert_eq!(game.cell_state(5, 5).content, Some(CellContent::Mine));
        // The Trigger Mine is the only one flagged as trigger.
        assert!(!game.is_trigger(5, 5));
    }

    #[test]
    fn reveal_shows_neighbor_count() {
        let mut game = Game::with_mines(Difficulty::Beginner, &[(2, 2)]);
        game.reveal(1, 1);
        assert_eq!(game.game_state(), GameState::Playing);
        assert_eq!(game.cell_state(1, 1).state, CellState::Revealed);
        assert_eq!(game.cell_state(1, 1).content, Some(CellContent::Number(1)));
    }

    #[test]
    fn zero_cell_flood_fills_until_numbered_boundary() {
        let mut game = Game::with_mines(Difficulty::Beginner, &[(4, 4)]);
        game.reveal(0, 0);
        // The clicked Cell is a zero Cell.
        assert_eq!(game.cell_state(0, 0).content, Some(CellContent::Number(0)));
        // The numbered boundary of the flood fill is Revealed.
        assert_eq!(game.cell_state(3, 3).content, Some(CellContent::Number(1)));
        // A far corner in the zero region is Revealed.
        assert_eq!(game.cell_state(8, 8).content, Some(CellContent::Number(0)));
        ...
        assert_eq!(game.cell_state(4, 4).state, CellState::Revealed);
        assert_eq!(game.cell_state(4, 4).content, Some(CellContent::Mine));
    }

    #[test]
    fn revealing_every_non_mine_cell_wins_and_reveals_mines() {
        let mut game = Game::with_mines(Difficulty::Beginner, &[(0, 0)]);
        let (width, height) = Difficulty::Beginner.size();
        for row in 0..height {
            for col in 0..width {
                if (row, col) != (0, 0) {
    ...
    fn ended_game_rejects_reveals() {
        ...
        game.reveal(1, 1);
        assert_eq!(game.game_state(), GameState::Lost);
        assert_eq!(game.cell_state(1, 1).state, CellState::Hidden);
    }
    ...
    fn flag_toggles_hidden_to_flagged_and_back() {
        ...
        assert_eq!(game.cell_state(1, 1).state, CellState::Flagged);
        game.toggle_flag(1, 1);
        assert_eq!(game.cell_state(1, 1).state, CellState::Hidden);
    }
    ...
    fn flagged_cell_blocks_reveal() {
        ...
        assert_eq!(game.game_state(), GameState::Ready);
        assert_eq!(game.cell_state(0, 0).state, CellState::Flagged);
    ...
    fn chord_sweeps_unflagged_neighbors_when_flags_match() {
        ...
        assert_eq!(game.cell_state(0, 1).state, CellState::Revealed);
        assert_eq!(game.cell_state(1, 0).state, CellState::Revealed);
        assert_eq!(game.cell_state(0, 0).state, CellState::Flagged);
    }
    ...
    fn chord_is_noop_when_flag_count_mismatches() {
        let mut game = Game::with_mines(Difficulty::Beginner, &[(0, 0)]);
        game.reveal(1, 1);
        game.chord(1, 1); // zero Flags around a 1
        assert_eq!(game.cell_state(0, 1).state, CellState::Hidden);
        assert_eq!(game.cell_state(1, 0).state, CellState::Hidden);
        assert_eq!(game.game_state(), GameState::Playing);
    }
    ...
    fn chord_is_noop_on_hidden_and_zero_cells() {
        ...
        game.chord(2, 2);
        assert_eq!(game.cell_state(2, 2).state, CellState::Hidden);
        // Zero Cell: nothing happens either.
        game.reveal(1, 1); // already revealed; reveal a zero region first
        assert_eq!(game.game_state(), GameState::Playing);
    }
    ...
    fn chord_hitting_a_mine_loses_with_that_mine_as_trigger() {
        ...
        let mut game = Game::with_mines(Difficulty::Beginner, &[(0, 0), (0, 2)]);
        game.reveal(1, 1);
        assert_eq!(game.cell_state(1, 1).content, Some(CellContent::Number(2)));
        game.toggle_flag(0, 0);
        game.toggle_flag(0, 1);
        game.chord(1, 1);
        assert_eq!(game.game_state(), GameState::Lost);
        assert!(game.is_trigger(0, 2));
        assert_eq!(game.cell_state(0, 2).content, Some(CellContent::Mine));
    }
