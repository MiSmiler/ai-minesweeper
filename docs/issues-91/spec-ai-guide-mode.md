...
- 左下仪表盘：**分析**按钮、「输入格式」下拉（4 种呈现形式）、「**session 策略**」下拉（预留：仅 `per-analysis` 可用，`per-game` 档标注「(未实现)」置灰）、**行列号**辅助标记 checkbox、**历史**列表。
...
- **前置失败**（HTTP 4xx/5xx、未流内容）→ 浏览器 alert 弹窗（阻塞、确定），文案按 bucket 分：
...
8. 作为玩家，我想点「**分析**」把当前局发给 AI，这样它能针对这盘真实的局面给建议（模型由后端按所选输入格式自动定：文本→flash、图像→vision-exp）。
...
35. 作为玩家，当 AI 请求**前置失败**（HTTP 400/401/402/422/429/500/503 或网络错误、未流内容）时，我想看到**按 bucket 分文案的 alert 弹窗**（阻塞、确定），这样我知道是配置错了还是上游临时抽风。
...
- `AiGuide` 组合分**三区**：
  ...
  - **左下仪表盘**：「分析」按钮（含「中断」）、「输入格式」下拉、「**session 策略**」下拉（预留：`per-analysis` 可用 / `per-game` 标注「(未实现)」置灰）、**行列号** checkbox、**历史**列表。
...
- 呈现分两型：
  - **①前置失败**（HTTP 4xx/5xx、未流内容）= 浏览器 alert 弹窗（阻塞、确定）。
    bucket 文案：配置错误 `400/401/402` | 上游瞬时 `429/500/503/网络`。走 HTTP 状态码 + 结构化错误体。
...
- **已定案**：`#96` 的「输入格式(8项)」是 **4 种形式（#94 权威）× 2 种 session 策略** 的误记。
  session 策略：`per-analysis`（一次分析一个全新上下文）/ `per-game`（同一 session 内多次分析拼接上下文）。
  本 map **只实现 `per-analysis`**（现状：每分析新上下文）。UI 预留「session 策略」下拉——**不与「输入格式」混**，
  两档均显示，但 `per-game` 档标注「(未实现)」并置灰不可选。**切换 session 策略 = 弃局开新局**（同切 mode 语义）。
  `per-game` 为将来「同 session 多次分析拼接上下文」留口。
