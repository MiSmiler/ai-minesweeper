...
> **Path migration**: tickets previously lived at `.scratch/<feature-slug>/issues/` and now live at `.scratch/<feature-slug>/tickets/`. Any `issues/` path you find in the repo — old tickets, docs, scripts — is stale legacy from the old layout; write new tickets under `tickets/`.
