...
The one-shot advisor mode (issue #91's "AI 看我玩") is `ai_adapter::suggest()`: single-turn, read-only, it injects the board and returns one analysis plus a suggestion. It is *not* named `assist` — that was only a working term in the issue discussion, not a code or doc symbol. The AI replies with its reasoning plus a trailing `(row,col)` coordinate (ADR-0016); that coordinate is unconsumed — reported but never parsed. The future persistent-loop mode (`AiPlay`) is `ai_play` and will run a tool loop that reads and writes the `Game`; its name is provisional.
